'use client'
import { getCourseReviws, getReviewsPercantage } from '@/actions/review.action'
...
function AllReviews(course: ICourse) {
	...
	const [percentages, setPercetages] = useState<{ [key: string]: number }>()
	...
	const onOpenReviews = async () => {
		...
		try {
			const [reviews, percentages] = await Promise.all([
				getCourseReviws(course._id, count),
				getReviewsPercantage(course._id),
			])
			setReviews(reviews)
			setPercetages(percentages)
			setLoading(false)
			setOpen(true)
		} catch (error) {
			setLoading(false)
	...
	const onLoadMore = async () => {
		...
		setLoading(true)
		try {
			const reviews = await getCourseReviws(course._id, newCount)
			setReviews(reviews)
			setLoading(false)
		} catch (error) {
			setLoading(false)
				...
				<DialogContent className='custom-scrollbar max-h-full max-w-full overflow-y-auto md:max-w-4xl'>
					<div className='mt-2 flex items-center gap-1'>
						...
						<div className='text-xl font-medium'>
							{t('reviewCourse')} : {course.rating.toString().slice(0, 4)}
						</div>
						<Dot />
						<div className='text-xl font-medium'>
							...
							{percentages &&
								[5, 4, 3, 2, 1].map(idx => (
									<div key={idx} className='flex items-center gap-2 '>
										<div className='relative mt-1 h-3 w-[60%] rounded bg-gray-200 dark:bg-gray-900 md:w-[40%]'>
											<div
												className=' absolute inset-0 rounded bg-gray-900 dark:bg-gray-200'
