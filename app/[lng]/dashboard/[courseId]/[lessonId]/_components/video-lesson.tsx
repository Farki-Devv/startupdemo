...
function VideoLesson({ lesson }: Props) {
	...
	const onEnd = async () => {
		...
		const nextLesson = getNextLesson(lesson._id, `${courseId}`).then(res =>
			router.push(`/dashboard/${courseId}/${res?.lessonId}?=${res?.sectionId}`)
		)
		const complated = completeLesson(lesson._id, userId!, pathname)
		const promise = Promise.all([nextLesson, complated])
		toast.promise(promise, {
			loading: t('loading'),
			success: t('succesfully'),
			error: t('error'),
		})
	}
